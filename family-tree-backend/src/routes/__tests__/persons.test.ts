...
describe('Person API Endpoints', () => {
  ...
  describe('GET /api/persons', () => {
    ...
    it('should return all persons when they exist', async () => {
      ...
      await storage.writeJSON(`persons/${person1.id}.json`, person1);
      await storage.writeJSON(`persons/${person2.id}.json`, person2);
      ...
      const returnedIds = response.body.data.map((p: Person) => p.id);
  ...
  describe('GET /api/persons/:id', () => {
    it('should return person when valid ID is provided', async () => {
      ...
      await storage.writeJSON(`persons/${person.id}.json`, person);
  ...
  describe('POST /api/persons', () => {
    ...
    it('should create person with parent relationships', async () => {
      ...
      const updatedParent = await storage.readJSON<Person>(`persons/${parent.id}.json`);
    ...
    it('should return 400 when more than 2 parents are provided', async () => {
      const personData = {
        firstName: 'Child',
        lastName: 'Doe',
        parentIds: [uuidv4(), uuidv4(), uuidv4()]
      };
  ...
  describe('DELETE /api/persons/:id', () => {
    it('should delete person and clean up relationships', async () => {
      ...
      await storage.writeJSON(`persons/${parent.id}.json`, parent);
      await storage.writeJSON(`persons/${child.id}.json`, child);
      await storage.writeJSON(`persons/${person.id}.json`, person);
      ...
      const deletedPerson = await storage.readJSON<Person>(`persons/${person.id}.json`);
      expect(deletedPerson).toBeNull();

      // Check that relationships were cleaned up
      const updatedParent = await storage.readJSON<Person>(`persons/${parent.id}.json`);
      const updatedChild = await storage.readJSON<Person>(`persons/${child.id}.json`);
  ...
  describe('POST /api/persons/:id/children', () => {
    ...
    it('should return 400 when child already has 2 parents', async () => {
      ...
      await storage.writeJSON(`persons/${parent1.id}.json`, parent1);
      await storage.writeJSON(`persons/${parent2.id}.json`, parent2);
      await storage.writeJSON(`persons/${parent3.id}.json`, parent3);
      await storage.writeJSON(`persons/${child.id}.json`, child);
