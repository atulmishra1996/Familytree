...
export class RelationshipUtils {
   ...
   */
  static addChildRelationship(parent: Person, child: Person): { parent: Person; child: Person } {
    const updatedParent = {
      ...parent,
   ...
   */
  static removeChildRelationship(parent: Person, child: Person): { parent: Person; child: Person } {
    const updatedParent = {
      ...parent,
  ...
  static hasCircularRelationship(persons: Person[], parentId: string, childId: string): boolean {
    ...
    const checkAncestors = (personId: string): boolean => {
      if (visited.has(personId)) return false;
      if (personId === childId) return true;
   ...
   */
  static validateTreeIntegrity(persons: Person[]): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
